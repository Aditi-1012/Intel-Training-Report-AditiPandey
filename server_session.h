...
ServerSession::ServerSession(const ServerSession& other, unsigned int id, unsigned int incomingCounter, unsigned int outgoingCounter, unsigned int state)
    : Session(other)
{
    _nextSessionId = 1;
    _state = state;
...
Session::ReceiveResult ServerSession::receiveMessage(BYTE* buffer, size_t bufferSize, unsigned int timeout_sec, BYTE** ppPayload, size_t* pPayloadSize, ServerSession** ppChildSession, unsigned int* pChildSessionId)
{
    ...
    if (header->sessionId == 0) 
    {
        ...
        if (message2 == NULL)
        {
            printf("Error during receive - error preparing Sigma message 2\n");
            newSession->cleanDhData();
            delete newSession;
            return RR_FATAL_ERROR;
        }

        if (!newSession->sendMessageInternal(HELLO_BACK_SESSION_MESSAGE, message2.get(), message2.size()))
        {
        ...
        _nextSessionId++;
        auto ret = _activeSessions.insert(std::pair<unsigned int, ServerSession*>(newSession->_sessionId, newSession));
        auto it = ret.first;
