...
#define MESSAGE_BUFFER_SIZE_BYTES 1000
#define HMAC_SIZE_BYTES 32
#define SYMMETRIC_KEY_SIZE_BYTES 32
#define SIGNATURE_SIZE_BYTES 384
#define DH_KEY_SIZE_BYTES 384
...
bool testSymmetricEncryption()
{
    ...
    size_t plaintextSize = strnlen(originalPlaintext, MESSAGE_BUFFER_SIZE_BYTES) + 1;
        
    // generate a random key
    // *********************
    if (!deriveKeyFromRandom(secretKey, SYMMETRIC_KEY_SIZE_BYTES, (const BYTE*)keyContext, strnlen(keyContext, 1000)))
    {
...
bool testRsaSigning(KeypairContext* privateKeyContext, KeypairContext* publicKeyContext, bool toModifyMessage)
{
    ...
    size_t messageSize = strnlen(message, MESSAGE_BUFFER_SIZE_BYTES) + 1;
...
bool testDh()
{
    ...
    if (!CryptoWrapper::startDh(&dhBobContext, bobPublicKeyBuffer, DH_KEY_SIZE_BYTES))
        ok = false;
